package com.xenon.client.entity;

import com.mojang.authlib.GameProfile;
import com.xenon.XenonClient;
import com.xenon.util.GameProfileParsing;
...
public class EntityOtherPlayerMP extends AbstractClientPlayer {
    ...
    protected final boolean forgeUser;
    ...
    public EntityOtherPlayerMP(World worldIn, GameProfile gameProfileIn) {
        ...
        if (XenonClient.instance.settings.revealNick) {
            Optional<Map.Entry<UUID, String>> res = GameProfileParsing.gameProfileTextureToUUIDAndProfileName(
                    gameProfileIn
            );
            if (res.isPresent()) {
                Map.Entry<UUID, String> re = res.get();
                String realName = re.getValue();
                if ( !gameProfileIn.getName().contains(realName) )
                    XenonClient.instance.sendmsg(RED.toString() + BOLD + gameProfileIn.getName() +
                            " is nicked! Possible IGN is: '" + realName + "' (UUID: " + re.getKey() + ')');
            }
        }

        forgeUser = ! gameProfileIn.getProperties().get("forgeClient").isEmpty();
        this.realName = null;
    }
    ...
    public void onUpdate() {
        ...
        float f = Math.min((float) Math.sqrt(d0 * d0 + d1 * d1) * 4.0F, 1);
    ...
    public void onLivingUpdate() {
        if (this.otherPlayerMPPosRotationIncrements > 0) {
            ...
            if (d3 < -180.0) {
                int i = (int) d3;
                int q = (-180 - i + 360 - 1) / 360;
                d3 += 360 * q;

            } else if (d3 > 180.0) {
                int i = (int) d3;
                int q = (i - 180 + 360 - 1) / 360;
                d3 -= 360 * q;
            }

            /*
            lame vanilla computation
