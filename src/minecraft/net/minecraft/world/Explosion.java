...
public class Explosion
{
    ...
    private final PRNG explosionRNG;
            ...
            if (!(value).isImmuneToExplosions()) {
                ...
                if (d12 <= 1.0D) {
                    ...
                    if (d13 != 0.0D) {
                        ...
                        (value).attackEntityFrom(DamageSource.setExplosionSource(this), (float) ((int) ((d10 * d10 + d10) / 2.0D * 8.0D * (double) f3 + 1.0D)));
                        double d11 = EnchantmentProtection.func_92092_a(value, d10);
                        (value).motionX += d5 * d11;
                        (value).motionY += d7 * d11;
                        (value).motionZ += d9 * d11;

                        if (value instanceof EntityPlayer && !((EntityPlayer) value).capabilities.disableDamage) {
                            this.playerKnockbackMap.put((EntityPlayer) value, new Vec3(d5 * d10, d7 * d10, d9 * d10));
                        }
                    }
                }
            }
        ...
        if (this.isSmoking)
        {
            for (BlockPos blockpos : this.affectedBlockPositions)
            {
                ...
                if (spawnParticles)
                {
                    ...
                    d4 = d4 * d7;
                    d5 = d5 * d7;
                    this.worldObj.spawnParticle(EnumParticleTypes.EXPLOSION_NORMAL, (d0 + this.explosionX) / 2.0D, (d1 + this.explosionY * 1.0D) / 2.0D, (d2 + this.explosionZ * 1.0D) / 2.0D, d3, d4, d5, new int[0]);
