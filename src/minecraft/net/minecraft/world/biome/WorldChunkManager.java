...
public class WorldChunkManager
{
    ...
    protected WorldChunkManager()
    {
        ...
        this.biomesToSpawnIn.add(Biomes.forest);
        this.biomesToSpawnIn.add(Biomes.plains);
        this.biomesToSpawnIn.add(Biomes.taiga);
        this.biomesToSpawnIn.add(Biomes.taigaHills);
        this.biomesToSpawnIn.add(Biomes.forestHills);
        this.biomesToSpawnIn.add(Biomes.jungle);
        this.biomesToSpawnIn.add(Biomes.jungleHills);
    }
    ...
    public void getRainfall(float[] listToReuse, int x, int z, int width, int length)
    {
        ...
        for (int i = 0; i < width * length; ++i)
        {
            try
            {
                float f = (float)BiomeGenBase.getBiomeFromBiomeList(aint[i], Biomes.ocean).getIntRainfall() / 65536.0F;
    ...
    public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] biomes, int x, int z, int width, int height)
    {
        ...
        try
        {
            ...
            {
                biomes[i] = BiomeGenBase.getBiomeFromBiomeList(aint[i], Biomes.ocean);
            }
    ...
    public BiomeGenBase[] getBiomeGenAt(BiomeGenBase[] listToReuse, int x, int z, int width, int length, boolean cacheFlag)
    {
        ...
        else
        {
            ...
            for (int i = 0; i < width * length; ++i)
            {
                listToReuse[i] = BiomeGenBase.getBiomeFromBiomeList(aint[i], Biomes.ocean);
            }
    ...
    public BlockPos findBiomePosition(int x, int z, int range, List<BiomeGenBase> biomes, PRNG random)
    {
        IntCache.resetIntCache();
        int i = x - range >> 2;
        int j = z - range >> 2;
