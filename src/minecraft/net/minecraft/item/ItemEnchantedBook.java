...
public class ItemEnchantedBook extends Item {
        ...
        if (nbttaglist != null) {
            for (int i = 0; i < nbttaglist.tagCount(); ++i) {
                ...
                if (Enchants.getEnchantmentById(j) != null) {
                    tooltip.add(Enchants.getEnchantmentById(j).getTranslatedName(k));
                }
            }
        }
    }
    ...
    public WeightedRandomChestContent getRandom(PRNG rand) {
        return this.getRandom(rand, 1, 1, 1);
    }

    public WeightedRandomChestContent getRandom(PRNG rand, int minChance, int maxChance, int weight) {
