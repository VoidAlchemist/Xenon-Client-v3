...
public final class ItemStack {
     ...
     * negated.
     */
    public boolean attemptDamageItem(int amount, PRNG rand) {
        if (!this.isItemStackDamageable()) {
            return false;
        } else {
            if (amount > 0) {
                int i = EnchantmentHelper.getEnchantmentLevel(Enchants.unbreaking.effectId, this);
                int j = 0;
    ...
    public List<String> getTooltip(EntityPlayer playerIn, boolean advanced) {
        if (XenonClient.instance.settings.enhancedToolTip)
            return TextAPI.getTooltip(this, playerIn);
            ...
            if ((i1 & 1) == 0) {
                ...
                if (nbttaglist != null) {
                    for (int j = 0; j < nbttaglist.tagCount(); ++j) {
                        ...
                        if (Enchants.getEnchantmentById(k) != null) {
                            list.add(Enchants.getEnchantmentById(k).getTranslatedName(l));
                        }
                    }
                }
            }
