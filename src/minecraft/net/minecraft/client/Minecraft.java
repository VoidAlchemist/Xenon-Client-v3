...
import com.xenon.client.gui.GuiMainMenuXenon;
import com.xenon.modules.api.MinecraftAPI;
import com.xenon.modules.api.PhantomCamAPI;
import com.xenon.modules.api.XenonEntityRenderer;
import com.xenon.modules.api.XenonMovementInput;
...
public class Minecraft implements IThreadListener, IPlayerUsage {
    ...
    private void startGame() throws LWJGLException, IOException {
        ...
        XenonClient.instance.init();
        XenonClient.instance.splashScreen.setProgress("MC | Processing Resources");
        this.initStream();
        ...
        XenonClient.instance.splashScreen.setProgress("MC | Model Manager");
        ...
        XenonClient.instance.splashScreen.setProgress("MC | Render Item");

        this.renderItem = new RenderItem(this.renderEngine, modelManager);

        XenonClient.instance.splashScreen.setProgress("MC | Render Manager");

        this.renderManager = new RenderManager(this.renderEngine, this.renderItem);

        XenonClient.instance.splashScreen.setProgress("MC | Item Renderer");

        this.itemRenderer = new ItemRenderer(this);
        this.mcResourceManager.registerReloadListener(this.renderItem);
        this.entityRenderer = new XenonEntityRenderer(this, this.mcResourceManager);
        this.mcResourceManager.registerReloadListener(this.entityRenderer);

        XenonClient.instance.splashScreen.setProgress("MC | Block Renderer");

        this.blockRenderDispatcher = new BlockRendererDispatcher(modelManager.getBlockModelShapes(), this.gameSettings);
        this.mcResourceManager.registerReloadListener(this.blockRenderDispatcher);

        XenonClient.instance.splashScreen.setProgress("MC | Render Global");
        ...
        XenonClient.instance.splashScreen.setProgress("MC | Finishing...");

        this.checkGLError("Post startup");
        this.ingameGUI = new GuiIngame(this);

        if (this.serverName != null) {
            this.displayGuiScreen(new GuiConnecting(new GuiMainMenuXenon(), this, this.serverName, this.serverPort));
        } else {
            this.displayGuiScreen(new GuiMainMenuXenon());
        }

        XenonClient.instance.postInit();
    ...
    private void createDisplay() throws LWJGLException {
        ...
        Display.setTitle("Xenon Client 2.0 | MC 1.8.9");

        try {
            Display.create((new PixelFormat()).withDepthBits(24));
        } catch (LWJGLException lwjglexception) {
            logger.error((String) "Couldn't set pixel format",  lwjglexception);
    ...
    private void setWindowIcon() {
        ...
        if (util$enumos != Util.EnumOS.OSX) {
            ...
            try {
                ...
                logger.error((String) "Couldn't set icon",  ioexception);
            } finally {
    ...
    public void refreshResources() {
        ...
        try {
            ...
            logger.info((String) "Caught error stitching, removing all assigned resourcepacks",  runtimeexception);
            list.clear();
    ...
    public void displayGuiScreen(GuiScreen guiScreenIn) {
        ...
        if (guiScreenIn == null && this.theWorld == null) {
            guiScreenIn = new GuiMainMenuXenon();
        } else if (guiScreenIn == null && this.thePlayer.getHealth() <= 0.0F) {
            guiScreenIn = new GuiGameOver();
        }

        if (guiScreenIn instanceof GuiMainMenuXenon) {
    ...
    private void runGameLoop() throws IOException {
        ...
        while (getSystemTime() >= this.debugUpdateTime + 1000L) {
            ...
            this.debug = String.format("%d fps (%d chunk update%s) T: %s%s%s%s%s",
                    debugFPS, RenderChunk.renderChunksUpdated, RenderChunk.renderChunksUpdated != 1 ?
    ...
    private void sendClickBlockToController(boolean leftClick) {
        MinecraftAPI.sendClickBlockToController(this, leftClick);
    }

    private void clickMouse() {
        if (XenonClient.instance.settings.oldHitreg)
            MinecraftAPI.clickMouse(this);
        else {
    ...
    public void rightClickMouse() {
        if (!this.playerController.getIsHittingBlock()) {
            this.rightClickDelayTimer = XenonClient.instance.settings.placeDelay;
            boolean flag = true;
            ...
            if (this.objectMouseOver == null) {
                ...
                switch (this.objectMouseOver.typeOfHit) {
                    case ENTITY:
                        flag = !( playerController.isPlayerRightClickingOnEntity(
                                this.thePlayer, this.objectMouseOver.entityHit, this.objectMouseOver
                        ) || playerController.interactWithEntitySendPacket(
    ...
    private void runTick() throws IOException {
        ...
        if (this.currentScreen == null || this.currentScreen.allowUserInput) {
            ...
            if (!PhantomCamAPI.enabled) {
                ...
                if (this.gameSettings.keyBindDrop.isPressed()) {
                    if (!this.thePlayer.isSpectator()) {
                        this.thePlayer.dropOneItem(GuiScreen.isCtrlKeyDown());
                    }
                }

                if (this.gameSettings.keyBindChat.isPressed() && flag) {
                    this.displayGuiScreen(new GuiChat());
                }

                if (this.currentScreen == null && flag) {
    ...
    public void loadWorld(WorldClient worldClientIn, String loadingMessage) {
        ...
        if (worldClientIn != null) {
            ...
            this.thePlayer.movementInput = new XenonMovementInput(this.gameSettings);
