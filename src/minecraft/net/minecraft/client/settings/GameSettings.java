...
public class GameSettings {
    ...
    public KeyBinding xenonBindModules,
            xenonBindSprintToggled,
            xenonBindSneakToggled,
            xenonBindDisconnect,
            xenonProfileSwitcher,
            xenonGlowingToggled,
            xenonPhantomCamToggled;
    ...
    public GameSettings(Minecraft mcIn, File optionsFileIn) {
        this.keyBindings = ArrayUtils.addAll(new KeyBinding[]{this.keyBindAttack, this.keyBindUseItem, this.keyBindForward,
                this.keyBindLeft, this.keyBindBack, this.keyBindRight, this.keyBindJump, this.keyBindSneak,
                this.keyBindSprint, this.keyBindDrop, this.keyBindInventory, this.keyBindChat, this.keyBindPlayerList,
                this.keyBindPickBlock, this.keyBindCommand, this.keyBindScreenshot, this.keyBindTogglePerspective, this.keyBindSmoothCamera,
                this.keyBindStreamStartStop, this.keyBindStreamPauseUnpause, this.keyBindStreamCommercials, this.keyBindStreamToggleMic,
        ...
        this.keyBindings = ArrayUtils.addAll(new KeyBinding[]{
                xenonBindModules = new KeyBinding("HUD Settings", Keyboard.KEY_RSHIFT, "Xenon Client"),
                xenonBindSprintToggled = new KeyBinding("Toggle sprint", Keyboard.KEY_LCONTROL, "Xenon Client"),
                xenonBindSneakToggled = new KeyBinding("Toggle sneak", Keyboard.KEY_V, "Xenon Client"),
                xenonBindDisconnect = new KeyBinding("Macro disconnect", Keyboard.KEY_U, "Xenon Client"),
                xenonProfileSwitcher = new KeyBinding("Macro mode switcher", Keyboard.KEY_P, "Xenon Client"),
                xenonGlowingToggled = new KeyBinding("Toggle Glowing Entities", Keyboard.KEY_R, "Xenon Client"),
                xenonPhantomCamToggled = new KeyBinding("Toggle Phantom Camera", Keyboard.KEY_Y, "Xenon Client")},
                this.keyBindings);
    ...
    public static String getKeyDisplayString(int key) {
        return key < 0 ? I18n.format("key.mouseButton", key + 101) : (key < 256 ? Keyboard.getKeyName(key) : String.format("%c",
                (char) (key - 256)).toUpperCase());
    }
                                                                                                ...
                                                                                                (settingOption == GameSettings.Options.MIPMAP_LEVELS ? (f1 == 0.0F ? s1 + I18n.format("options.off") :
                                                                                                        ...
                                                                                                        (settingOption == GameSettings.Options.STREAM_KBPS ? s1 + TwitchStream.formatStreamKbps(f) + " Kbps" :
                                                                                                                (settingOption == GameSettings.Options.STREAM_BYTES_PER_PIXEL ? s1 + String.format("%.3f bpp",
                                                                                                                        TwitchStream.formatStreamBps(f)) : (f == 0.0F ? s1 + I18n.format("options.off") :
                                                                                                                        s1 + (int) (f * 100.0F) + "%"))))))))))))))));
    ...
    public void loadOptions() {
        ...
        label2:
        {
            try {
                ...
                gson.error((String) "Failed to load options",  exception1);
                break label2;
            } finally {
    ...
    public void saveOptions() {
        try {
            ...
            gson.error((String) "Failed to save options",  exception);
        }
    ...
    public float getSoundLevel(SoundCategory sndCategory) {
        return this.optionsFile.getOrDefault(sndCategory, 1.0F);
    }
