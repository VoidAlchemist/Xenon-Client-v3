...
public class EntityRenderer implements IResourceManagerReloadListener {
    ...
    private final PRNG random = new PRNG();
    ...
    private void loadShader(ResourceLocation resourceLocationIn) {
        if (OpenGlHelper.isFramebufferEnabled()) {
            try {
                ...
                logger.warn((String) ("Failed to load shader: " + resourceLocationIn),  ioexception);
                this.shaderIndex = shaderCount;
                this.useShader = false;
            }
        }
        ...
        if (entity != null && this.mc.theWorld != null) {
            ...
            for (Entity value : list) {
                ...
                if (axisalignedbb.isVecInside(vec3)) {
                    ...
                    if (d3 < d2 || d2 == 0.0D) {
                        if (value == entity.ridingEntity && d2 == 0.0D) {
                            pointedEntity = value;
    ...
    public void setupCameraTransform(float partialTicks, int pass) {
        ...
        if (f1 > 0.0F) {
            ...
            if (this.mc.thePlayer.isPotionActive(Potions.confusion)) {
                i = 7;
            }
    ...
    protected void updateLightmap(float partialTicks) {
        if (this.lightmapUpdateNeeded) {
            ...
            if (world != null) {
                if (Config.isCustomColors() && CustomColors.updateLightmap(world, this.torchFlickerX, this.lightmapColors, this.mc.thePlayer.isPotionActive(Potions.nightVision), partialTicks)) {
                ...
                for (int i = 0; i < 256; ++i) {
                    ...
                    if (this.mc.thePlayer.isPotionActive(Potions.nightVision)) {
    ...
    public float getNightVisionBrightness(EntityLivingBase entitylivingbaseIn, float partialTicks) {
        int i = entitylivingbaseIn.getActivePotionEffect(Potions.nightVision).getDuration();
    ...
    public void updateCameraAndRender(float partialTicks, long nanoTime) {
        ...
        if (this.mc.inGameHasFocus && flag) {
            ...
            if (this.mc.gameSettings.smoothCamera) {
                ...
                (PhantomCamAPI.enabled ? PhantomCamAPI.camera : mc.thePlayer).setAngles(f2, f3 * (float) i);
            } else {
                this.smoothCamYaw = 0.0F;
                this.smoothCamPitch = 0.0F;
                (PhantomCamAPI.enabled ? PhantomCamAPI.camera : mc.thePlayer).setAngles(f2, f3 * (float) i);
                    ...
                    if (biomegenbase.canRain() || biomegenbase.getEnableSnow()) {
                        ...
                        if (k2 != l2) {
                            ...
                            if (world.getWorldChunkManager().getTemperatureAtHeight(f1, j2) >= 0.15F) {
                                ...
                                float f2 = (float)Math.sqrt(d6 * d6 + d7 * d7) / (float) i1;
                                ...
                                float f6 = (float)Math.sqrt(d11 * d11 + d12 * d12) / (float) i1;
    ...
    private void updateFogColor(float partialTicks) {
        ...
        if (this.cloudFog) {
            ...
            if (entity instanceof EntityLivingBase && ((EntityLivingBase) entity).isPotionActive(Potions.waterBreathing)) {
                f12 = f12 * 0.3F + 0.6F;
            }
        ...
        if (XenonClient.instance.settings.blindness &&
                entity instanceof EntityLivingBase && ((EntityLivingBase) entity).isPotionActive(Potions.blindness)) {
            int i = ((EntityLivingBase) entity).getActivePotionEffect(Potions.blindness).getDuration();
        ...
        if (entity instanceof EntityLivingBase && ((EntityLivingBase) entity).isPotionActive(Potions.nightVision)) {
