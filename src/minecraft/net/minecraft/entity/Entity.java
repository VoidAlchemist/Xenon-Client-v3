...
public abstract class Entity implements ICommandSender {
    ...
    public void moveEntity(double x, double y, double z) {
        if (this.noClip) {
            ...
            if (this.canTriggerWalking() && !flag && this.ridingEntity == null) {
                ...
                this.distanceWalkedModified = (float) ((double) this.distanceWalkedModified + (double) Math.sqrt(d12 * d12 + d14 * d14) * 0.6D);
                this.distanceWalkedOnStepModified = (float) ((double) this.distanceWalkedOnStepModified + (double) Math.sqrt(d12 * d12 + d13 * d13 + d14 * d14) * 0.6D);
    ...
    protected void resetHeight() {
        float f = (float)Math.sqrt(this.motionX * this.motionX * 0.20000000298023224D + this.motionY * this.motionY + this.motionZ * this.motionZ * 0.20000000298023224D) * 0.2F;
    ...
    public boolean isEntityInsideOpaqueBlock() {
        if (this.noClip) {
            ...
            for (int i = 0; i < 8; ++i) {
                int j = MathHelper.floor_double(this.posY + (double) (((float) ((i) & 1) - 0.5F) * 0.1F) + (double) this.getEyeHeight());
                int k = MathHelper.floor_double(this.posX + (double) (((float) ((i >> 1) & 1) - 0.5F) * this.width * 0.8F));
                int l = MathHelper.floor_double(this.posZ + (double) (((float) ((i >> 2) & 1) - 0.5F) * this.width * 0.8F));
    ...
    public boolean isInvisibleToPlayer(EntityPlayer player) {
        return !XenonClient.instance.settings.showInvisibleEntities && !player.isSpectator() && this.isInvisible();
    }
    ...
    public String toString() {
        return String.format("%s['%s'/%d, l='%s', x=%.2f, y=%.2f, z=%.2f]",
                this.getClass().getSimpleName(), this.getName(), this.entityId, this.worldObj == null ?
