...
public class TileEntityBeacon extends TileEntityLockable implements ITickable, IInventory {
    ...
    public static final Potion[][] effectsList = new Potion[][]{
            {Potions.moveSpeed, Potions.digSpeed},
            {Potions.resistance, Potions.jump},
            {Potions.damageBoost}, {Potions.regeneration}};
    ...
    private int func_183001_h(int potionID) {
        if (potionID < 0 || potionID >= Potions.potionTypes.length)
            return 0;

        Potion pot = Potions.potionTypes[potionID];
        return pot == Potions.moveSpeed ||
                pot == Potions.digSpeed ||
                pot == Potions.resistance ||
                pot == Potions.jump ||
                pot == Potions.damageBoost ||
                pot == Potions.regeneration ? potionID : 0;
    }
